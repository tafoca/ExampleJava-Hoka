...
/**
 * A HTTP handler which creates a new user.
 */
public class CreateNewUser implements RequestHandler {
	...
	public void process(HttpRequest request, HttpResponse response) {
		try {
			...
			User exist = UsersDao.findUserById(id);
			if (exist != null) {
				halt(HttpConstants.HTTP_STATUS_FORBIDDEN, "user already exists"); //$NON-NLS-1$
			}
